...
/**
 * Wraps a facebook stream.
 * 
 * Develop only. This code should be fixed cause its messy.
 *
 */
public class StreamUi extends Composite implements ClickHandler {
    ...
    public Widget createLikesWidget ( Likes likes ) {
        ...
        String txt = null;
        if ( likes.isUserLikes() && likes.getCountExcludeUser() == 0 ) {
            txt = "You like this";
        }

        else if ( likes.isUserLikes() && likes.getCountExcludeUser() > 0 ) {
            txt = "You and " + likes.getCountExcludeUser() + " people likes this ";
        }
        else if ( likes.getCount() > 0 ) {
            txt = likes.getCount() + " people likes this";
        }
	...
	private Widget renderUI () {
		...
		if (stream.getLikes().getCount() > 0) {
			...
			likesPnl.add(createLikesWidget ( stream.getLikes() ) );
			inner.add(likesPnl);
		}
